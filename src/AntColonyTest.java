 ...
 * @author andav (Debug)
 */
public class AntColonyTest extends GraphicsProgram {
    ...
    public class Grid {
        ...
        public class Box extends MouseAdapter {
            ...
            public Box(int i, int j) {
                ...
                this.obj1 = new GRect((OFFSET + (i * SIZE)), (OFFSET + (j * SIZE)), SIZE, SIZE/2);
                this.obj2 = new GRect((OFFSET + (i * SIZE)), (OFFSET + (j * SIZE))+SIZE/2, SIZE, SIZE/2);
                this.test1 = new GLabel("" + trailH);
                this.test1.setLocation((OFFSET + (i * SIZE)), (OFFSET+10 + (j * SIZE)));
                this.test2 = new GLabel("" + trailF);
                this.test2.setLocation((OFFSET + (i * SIZE)), (OFFSET+10 + (j * SIZE))+SIZE/2);
                
                this.obj.addMouseListener(this);
                this.obj1.setColor(Color.white);
                this.obj1.setFilled(true);
                this.obj2.setColor(Color.white);
                this.obj2.setFilled(true);
            ...
            public void updateColor() {
                ...
                this.obj1.setColor(new Color(red,green,blue));
                this.obj1.setFilled(true);

                red = 255;
                green = 255;
                blue = 255 - (this.trailF * NEG_DIFF);
                this.obj2.setColor(new Color(red,green,blue));
                this.obj2.setFilled(true);

                this.test1.setLabel("" + trailH);
                this.test2.setLabel("" + trailF);
            }
            ...
            public void switchState(MouseEvent e) {
                if (SwingUtilities.isLeftMouseButton(e)) {
                    this.obj.setColor(Color.black);
                    this.obj.setFilled(true);
                } else {
                    this.obj.setFilled(false);
                }
            }
    ...
    public class Ant {
        ...
        public void wander() {
            ...
            for (int i = 0; i < direction.length; i++) {
                ...
                if (!((tempX < 0) || (tempX >= GRID_SIZE) || (tempY < 0) || (tempY >= GRID_SIZE)) && !(tiles.grid[tempX][tempY].obj.isFilled())) {
    ...
    public void addGrid() {
        for (int i = 0; i < this.tiles.grid.length; i++) {
            for (int j = 0; j < this.tiles.grid.length; j++) {
                this.add(this.tiles.grid[i][j].obj1);
                this.add(this.tiles.grid[i][j].obj2);
                this.add(this.tiles.grid[i][j].obj);
                
                this.add(this.tiles.grid[i][j].test1);
                this.add(this.tiles.grid[i][j].test2);
            }
        }
    }
